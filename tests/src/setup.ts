...
import { ChainOfCustodyClient } from '../../ui/src/chain-of-custody-client.js';
import { ChainOfCustodyStore } from '../../ui/src/chain-of-custody-store.js';

const testHappUrl = dirname(fileURLToPath(import.meta.url)) + '/../../workdir/chain-of-custody_test.happ';
...
async function addPlayer(scenario: Scenario) {
  ...
  const store = new ChainOfCustodyStore(
  	new ChainOfCustodyClient(player.appWs as any, 'chain_of_custody_test'),
  );
  return {
  	store,
  	player,
