...
export class CustodyTransfersForCustodyTransfer extends SignalWatcher(LitElement) {
  /**
  ...
  renderList(hashes: Array<ActionHash>) {
    if (hashes.length === 0) {
      return html` <div class="column placeholder center-content" style="gap: 8px; flex: 1">
        ...
        ></sl-icon>
        <span style="text-align: center">${msg("No custody transfers found for this custody transfer.")}</span>
      </div>`;
    }
  ...
  render() {
    const hashes = this.chainOfCustodyStore.custodyTransfers.get(this.custodyTransferHash).custodyTransfers.get();
